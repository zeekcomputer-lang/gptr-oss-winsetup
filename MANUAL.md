...
cp examples/sample-glossary.json data/glossary.json   # 또는 .env GPTR_GLOSSARY 지정
...
### 경로
- 명시: `.env` 에 `GPTR_GLOSSARY=./data/glossary.json` (또는 절대경로)
- 기본: 명시 없으면 `data/glossary.json` 이 있으면 자동 사용
- 둘 다 없으면 비활성(no-op) — 기존 동작 불변
- 크기 한도: `GPTR_GLOSSARY_MAX_KB`(기본 8). 초과분은 앞에서부터 포함하고 잘라낸 뒤 경고 주석을 남긴다.
...
### 사용
```bash
cp examples/sample-glossary.json data/glossary.json   # 예제를 기반으로 편집
