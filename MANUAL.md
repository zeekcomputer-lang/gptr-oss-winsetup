...
# 3) (선택) 용어사전 주입: 전문용어·고유명 정의를 사전에 주입(부록 F)
cp examples/sample-glossary.json data/glossary.json   # 파일로 관리(.env 아님). data/glossary/*.json 도 가능
python tools/launch.py glossary --show                 # 로드되는지 미리보기

# 4) 로컬 데이터 기반 보고서 생성 (--docx 면 표 포함 DOCX 동시 생성)
python tools/launch.py research "질의문" --source local --report-type detailed_report --docx

# 5) (따로) 기존 마크다운 → 비즈니스 DOCX(표 지원) 변환 — 부록 G
python tools/launch.py docx outputs/report.md
...
## 부록 G. DOCX 내보내기 — 비즈니스 보고서(표 지원)

출력 마크다운(.md)을 **세련된 비즈니스 DOCX** 로 변환한다. `tools/md_to_docx.py` —
deep-doc-pipeline-v2 의 변환기를 참조해 gptr-oss 출력에 맞게 **개선**한 버전.

### deepdoc-v2 대비 개선점
- ✅ **GFM 표**: `| a | b |` + 구분행 → 헤더 음영(네이비)·테두리·교대 행 음영·셀 정렬(`:--`,`--:`,`:-:`)
- ✅ 순서 목록(1. 2. 3.) + 중첩 들여쓰기 불릿/번호
- ✅ 펜스 코드블록(```) → 모노스페이스 음영 블록
- ✅ 링크 `[text](url)` → 실제 클릭 하이퍼링크(밑줄·강조색)
- ✅ H4 + 인라인(**굵게**/`코드`/링크) 통합 파서
- ✅ 표지(H1) + 자동 페이지번호 + 한글 폰트(맑은 고딕) 유지(deepdoc 계승)

### 사용
```bash
# 자동: research 와 함께(마크다운 + 같은 이름 .docx 동시 생성)
python tools/launch.py research "질의" --source local --docx     # 또는 .env GPTR_EXPORT_DOCX=1

# 수동: 기존 .md 파일 변환
python tools/launch.py docx outputs/report.md                  # report.docx
python tools/launch.py docx a.md b.md -o combined.docx         # 순서대로 병합(페이지 구분)
python tools/launch.py docx report.md --cover-label "기술 보고서"
```
Windows: `windows\docx.bat outputs\report.md`. python-docx 필요(`setup.py` 가 자동 설치, 순수 파이썬).

### 동작(내부)
- run_research 완료 → `_export_docx()` 가 `md_to_docx.build_report_docx(report, *.docx)` 호출(비치명: 실패해도 .md 유지).
- 파서는 줄 단위 상태머신 — 표/코드블록 같은 다중 행 블록을 선겵해 수집.

---

## 부록 F. 용어사전 주입 (전문지식 사전 참고)

요약/보고서가 고유명·전문용어·약어 정의를 필요로 할 때, **사전에 용어사전(JSON)을 주입**해
모델이 정의를 일관되게 쓰도록 한다. **RAG·chrono 두 모드 모두** 적용된다.

### 경로 — **.env 가 아니라 파일로 관리**(존재할 때만 활용)
- `data/glossary.json` (단일 파일, 권장)
- `data/glossary/*.json` (디렉터리 — 여러 파일을 파일명 오름차순으로 병합; 동일 용어는 먼저 등장한 정의 유지)
- 둘 다 없으면 비활성(no-op) — 기존 동작 불변. 경로/크기를 .env 로 지정하지 않는다.
- (data/ 는 git 제외 → 용어사전은 커밋되지 않는 사용자 데이터로 취급)
- 크기 한도는 8KB 상수. 초과분은 앞에서부터 포함하고 잘라낸 뒤 경고 주석을 남긴다.

### JSON 형식 (세 가지 모두 자동 판별)
```jsonc
// 1) terms 배열 (권장) — 별칭/안내문 포함
{ "instruction": "(선택) 모델에 줄 지시문",
  "terms": [ {"term":"BGE","definition":"...","aliases":["bge-m3"]} ] }
// 2) 평면 dict
{ "BGE": "...", "RAG": "..." }
// 3) 항목 배열
[ {"term":"KPI","definition":"..."} ]
```
키 별칭 허용: term=term|word|name|key, definition=definition|def|desc|meaning|value, aliases=aliases|alias|synonyms|abbr.

### 사용
```bash
cp examples/sample-glossary.json data/glossary.json   # 예제를 기반으로 편집(단일 파일)
# 또는 여러 파일로 나눠 관리: data/glossary/general.json, data/glossary/project.json ...
python tools/launch.py glossary --show                 # 로드/크기/샘플 확인 + 주입블록 전문
python tools/launch.py doctor                          # glossary 상태 한 줄 표시
```
...
### 본문 인라인 출처 억제 + 출처 정책 (기본 적용, chrono/rag 공통)
리서치가 local 모드에서 본문 중간에 임의 링크를 지어내고 문서ID(`[[id]]`)를 붙이는 문제를 2계층으로 차단한다.
- **계층 1(프롬프트 하드닝)**: `_patch_suppress_inline_citations` 가 `generate_report_*` 프롬프트에 "본문 인라인 링크/URL/ID 금지, 앞서의 APA·하이퍼링크 요구 무시" 지시문 덧붙(`GPTR_SUPPRESS_INLINE_CITATIONS`, 기본 ON).
- **계층 2(결정적 후처리, 필수)**: `tools/sanitize_report.py` 가 최종 .md 본문에서 인라인 링크→텍스트, 원시 URL·`[[id]]`·문서ID 제거. 펜스 코드블록·인라인 코드·표는 보호, 멱등.
- **출처 정책 `GPTR_SOURCE_MODE`**:
  - `none`(기본) = 본문 인라인 제거 + 출처 섹션도 제거 → 출처 일절 없음.
  - `end` = 본문 인라인 제거 + 맨 끝 단일 `## 출처` 섹션에 **문서 title 목록**만(중복제거·등장순).
- chrono 는 `[[id]]→title` 를 `data/digest/idmap.json`(`build_digest` 가 `idmap_out` 으로 생성) 으로 해석. **title 은 PDF/DOCX 의 경우 파일명**이라 의미 보장이 안 되므로 기본을 `none`으로 둔다(필요시 `end`).
- 수동: `python tools/sanitize_report.py outputs/report.md --mode end --idmap data/digest/idmap.json`. RAG 모드는 idmap 이 없어 `end` 출처는 best-effort(마커 없으면 생략).
