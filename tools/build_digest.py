...
레이트리밋/내성(게이트웨이 타임아웃 대응):
  ...
  - 일시 실패는 LLM_MAX_RETRIES 회 backoff 재시도. 출력 토큰 cap 기본은 SMART_TOKEN_LIMIT(8000) — 너무 낮추면 reduce 가 마커를 잘라 커버리지 손실.
누락 0 보장(2계층):
  - reduce 프롬프트가 [[id]] 마커 보존을 지시 + reduce 이후 digest 에서 사라진 문서는 combined 에서 해당 줄을 복원.
...
env:
  ...
  CHRONO_MAX_OUTPUT_TOKENS(기본=SMART_TOKEN_LIMIT 8000), LLM_TIMEOUT(기본 180), LLM_MAX_RETRIES(기본 2), GPTR_LLM_STREAM(기본 1),
...
def _out_tokens() -> int:
    """digest(map/reduce/추출) 출력 토큰 cap.
    ⚠ 기본을 너무 낮추면 reduce 가 이벤트·[[id]] 마커를 잘라내 커버리지가 무너진다.
    그래서 기본은 SMART_TOKEN_LIMIT(8000)을 따르고, 명시 설정 시에만 낮춘다.
    (낮추더라도 build_digest 의 'reduce 후 누락 복원'이 커버리지를 보장한다.)"""
    v = os.getenv("CHRONO_MAX_OUTPUT_TOKENS")
    if v and v.strip():
        return int(v)
    return int(os.getenv("SMART_TOKEN_LIMIT", "8000"))
...
_REDUCE_USER_TMPL = (
    ...
    "2) 각 항목 끝의 `[[id]]` 마커를 **절대 삭제하지 말고 그대로 보존**한다. "
    "서로 다른 id 는 합치지 말고, 같은 id 가 여러 번이면 한 항목으로 병합하되 그 id 는 남긴다.\n"
    "3) 시간 정보가 있으면 (YYYY-MM-DD) 로 시간순 정렬한다.\n"
    "4) 출력은 {lang} 불릿 목록만.\n\n"
    "주제(참고): {query}\n\n"
    "=== 요약 조각 시작 ===\n{chunks}\n=== 요약 조각 끝 ==="
)
...
def build_digest(doc_dir: Path, query: str, max_input_kb: int = 25,
    ...
    # reduce 전(combined) 커버리지 — 이 시점엔 전 문서 마커가 담겨있어야 한다.
    pre_reduce_missing = sorted(all_ids - found_ids(combined))

    # 예산 초과 시 재귀 reduce
    digest = reduce_text(combined, payload_budget, query)

    # ★ reduce 후 누락 복원 — 압축 과정에서 [[id]] 마커가 사라진 문서는
    # combined 의 해당 줄을 몇 구 붙여 복원한다(출력 cap · 과도 압축과 무관하게 누락 0 보장).
    dropped = sorted(all_ids - found_ids(digest))
    recovered_after_reduce = 0
    if dropped:
        ...
        for line in combined.splitlines():
            ...
            if not s:
                continue
            for i in found_ids(s):
                lines_by_id.setdefault(i, s)
        recover_lines = [lines_by_id[i] for i in dropped if i in lines_by_id]
        if recover_lines:
            digest = digest.rstrip() + "\n\n### (압축 중 누락 복원)\n" + "\n".join(recover_lines)
            recovered_after_reduce = len(recover_lines)
        if verbose:
            print(f"[digest][WARN] reduce 후 마커 소실 {len(dropped)}건 → "
                  f"combined 에서 {recovered_after_reduce}건 복원(누락 0 보장)")

    # 최종 누락 재계산(복원 반영)
    still_missing = sorted(all_ids - found_ids(digest))
    ...
    stats = {
        ...
        "pre_reduce_missing": len(pre_reduce_missing),
        "recovered_after_reduce": recovered_after_reduce,
