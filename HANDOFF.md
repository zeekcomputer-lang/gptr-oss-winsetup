...
| `tools/sanitize_report.py` | **신규** 최종 보고서 본문에서 인라인 출처(링크/URL/`[[id]]`/문서ID) 제거. 코드블록·표·말미 출처섹션 보호, 멱등, stdlib. `GPTR_SOURCE_MODE` 정책 |
...
- **digest 단계 게이트웨이 타임아웃/용량초과 대응(자가복구)**: map 호출이 용량초과/타임아웃으로 죽으면 `build_digest.map_batch` 가 배치를 반으로→단건→문서조각으로 **적응 분할 재시도**(커버리지 유지). reduce 도 `_reduce_piece_adaptive` 로 동일. 스트리밍(SSE, idle 504 회피·미지원시 비스트림 폴백), 일시실패 backoff(`LLM_MAX_RETRIES`), 타임아웃 노브(`LLM_TIMEOUT`=180). 근본 완화는 `CHRONO_MAX_INPUT_KB` 하향(12~16).
- **⚠ 출력 cap 과도 축소 금지(커버리지 회귀 교훈)**: `CHRONO_MAX_OUTPUT_TOKENS` 를 너무 낮게 잡으면 reduce 가 이벤트·`[[id]]` 마커를 잘라내 최종 digest 가 극소수 문서만 참조(60→2 사례). 그래서 기본을 SMART_TOKEN_LIMIT(8000)로 두고, **누락 0 을 2계층으로 보장**: (1) reduce 프롬프트가 `[[id]]` 마커 보존 지시 (2) reduce 후 digest 에서 사라진 문서는 `combined` 의 해당 줄을 복원(`### (압축 중 누락 복원)`). stats: `pre_reduce_missing`/`recovered_after_reduce`/`still_missing`(최종=0 목표).
...
- **본문 인라인 출처 억제(2계층, 기본 적용)**: local 모드에서 vendor 프롬프트가 인라인 하이퍼링크/APA 인용을 강제 → 모델이 본문에 임의 링크·문서ID(`[[id]]`)를 박는 문제 해소.
  - **계층 1(프롬프트 하드닝)**: `_patch_suppress_inline_citations`(patch) 가 `PromptFamily.generate_report_*` 에 "본문 인라인 링크/URL/ID 금지" 지시문을 덧붙임(`GPTR_SUPPRESS_INLINE_CITATIONS`, 기본 ON). 약체 모델 대비.
  - **계층 2(결정적 후처리, 필수)**: `tools/sanitize_report.py` 가 보고서 .md 의 본문에서 인라인 링크`[..](..)`→텍스트, 원시 URL·`[[id]]`·문서ID 제거. **펜스 코드블록/인라인 코드/표/말미 출처섹션 보호**, 멱등. run_research 후처리 훅(`_sanitize_report_hook`)에서 chrono/rag 공통 적용(용어집 부록보다 먼저).
  - **출처 정책 `GPTR_SOURCE_MODE`**: `none`(기본) = 인라인 제거 + 출처 섹션도 제거(출처 없음) / `end` = 인라인 제거 + 맨 끝 `## 출처` 에 **문서 title 목록**만. chrono 는 `[[id]]→title` 를 `data/digest/idmap.json`(build_digest 가 `idmap_out` 으로 생성) 으로 해석. title 은 파일명일 수 있어 의미 보장 안 됨 → 기본을 `none` 으로 둠.
  - 검증(mock, 2026-06-17): none/end 본문 링크·마커·URL 0 / 코드블록·인라인코드 보존 / end title 목록·순서·중복제거 / 멱등 / idmap 사이드카 — PASS.
